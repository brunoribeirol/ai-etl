...
**Last updated:** 2026-08-23 — **Roadmap complete: all 29 original sprints + all 9 Phase 2 sprints merged to `main` (Sprint 25/i18n was the last piece, PR #106).** `AI_ETL_SECRETS_ENCRYPTION_KEY` is now set in Railway (both `ai-etl` and `tranquil-appreciation`, redeployed to pick it up). **Migrations `0010`-`0020` (11 pending migrations, spanning Sprints 15-37) are now applied to production Supabase** — `alembic_version` confirmed at `0020` via direct SQL query. A real Alembic-chain fork (`0018`/`0019` both chaining onto `0017`, from two parallel-sprint PRs merging 10 minutes apart with no file-level conflict) was found and fixed first (PR #107) — applying migrations before that fix would have failed with "multiple head revisions."
