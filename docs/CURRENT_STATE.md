...
**Last updated:** 2026-08-22 (later session) — **Phase 2 ("Fase 2", Sprints 30-38, market-readiness hardening) all 9 of 9 sprints merged to `main` (PRs #94-#103), `v1.0.0` tagged.** Original 29-sprint roadmap unchanged from the summary below (28/29, only Sprint 25/i18n remains). Migrations `0010`-`0015` (original roadmap) and `0016`-`0019` (Phase 2) still not applied to production Supabase; `AI_ETL_SECRETS_ENCRYPTION_KEY` still not set in Railway — both still pending explicit owner confirmation, unchanged by this session's work.
