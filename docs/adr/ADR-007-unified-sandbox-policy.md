...
### Post-implementation addendum — child process environment

Sprint 2 review of the implementation (`review-agent-sprint2`) flagged that `multiprocessing`'s `spawn` context does not clear `os.environ` — the child inherits the parent's full environment verbatim, which includes this project's real secrets (`APP_DATABASE_URL`, `POSTGRES_URL`, `CLERK_JWKS_URL`/`CLERK_ISSUER`, `OPENAI_API_KEY`-style vars). Combined with the introspection bypass this ADR already accepts as an open limitation (`().__class__.__mro__[1].__subclasses__()` reaching an already-imported module's globals), this made those secrets a concrete, reachable target from inside the sandbox, without needing `import os` through the blocked builtins. This predates this ADR (in-process `exec()` had the same exposure, arguably worse, since the parent's live objects were also in the same address space) — the process boundary neither introduced nor fixed it on its own.

**Fixed, same PR**: `_sandbox_worker()` calls `os.environ.clear()` as its first line, before `SAFE_GLOBALS`/`extra_globals` are assembled and before any user code runs — inside the child only, never the parent (the parent process still needs its own environment for everything else the app does). Checked for runtime env-var dependencies in the code paths Transformer/Analyst/Science actually exercise (pandas/numpy operations, sklearn estimators at their default `n_jobs=None`, i.e. no joblib/loky parallel-worker temp-dir lookup) and found none that need preserving; no allowlist was necessary. This closes the env-var-exposure angle of the introspection limitation specifically — the introspection bypass itself (arbitrary code execution within the now-secret-free child process) remains an accepted, separate limitation. See `tests/unit/test_sandbox.py`'s env-clearing tests and `SECURITY.md`.
